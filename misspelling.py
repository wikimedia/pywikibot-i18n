# -*- coding: utf-8 -*-
msg = {
    'en': {
        'misspelling-fixing': u'Bot: Fixing misspelled link to %(page)s',
    },
    'qqq': {
        'misspelling-fixing': u'Edit summary when the bot fixes links that contain common spelling mistakes.',
    },
    'ar': {
        'misspelling-fixing': u'روبوت: إصلاح وصلة خاطئة إلى %(page)s',
    },
    # Author: Xuacu
    'ast': {
        'misspelling-fixing': u'Robot: Iguando l\'enllaz mal escritu a %(page)s',
    },
    # Author: Red Winged Duck
    'be-x-old': {
        'misspelling-fixing': u'Робат: выпраўленьне памылковай спасылкі на %(page)s',
    },
    # Author: Edinwiki
    'bs': {
        'misspelling-fixing': u'Bot: Popravljanje pogrešne veze prema %(page)s',
    },
    # Author: SMP
    'ca': {
        'misspelling-fixing': u'Robot: Corregint un enllaç mal escrit cap a %(page)s',
    },
    # Author: Matěj Suchánek
    'cs': {
        'misspelling-fixing': u'Robot: Opravuji pravopisně chybný odkaz na %(page)s',
    },
    'da': {
        'misspelling-fixing': u'Omdirigeringer af fejlstavninger: %(page)s',
    },
    'de': {
        'misspelling-fixing': u'Bot: korrigiere Link auf Falschschreibung: %(page)s',
    },
    # Author: Fitoschido
    'es': {
        'misspelling-fixing': u'Bot: corrección de enlace mal escrito hacia %(page)s',
    },
    # Author: Ebraminio
    'fa': {
        'misspelling-fixing': u'ربات: اصلاح پیوند با املاء نادرست به %(page)s',
    },
    # Author: Maxim21
    'fr': {
        'misspelling-fixing': u'Robot : Réparation des liens mal orthographiés vers %(page)s',
    },
    # Author: Murma174
    'frr': {
        'misspelling-fixing': u'Bot: Ferkiard skrewen ferwisang ferbeedert: %(page)s',
    },
    # Author: Toliño
    'gl': {
        'misspelling-fixing': u'Bot: Corrixo unha ligazón incorrecta cara a "%(page)s"',
    },
    # Author: Kolonahe
    'haw': {
        'misspelling-fixing': u'Lopako: Ke kāpili hou nei i ka loulou i pela hewa iā %(page)s',
    },
    'he': {
        'misspelling-fixing': u'בוט: מתקן קישור עם שגיאה לדף %(page)s',
    },
    # Author: C5st4wr6ch
    'id': {
        'misspelling-fixing': u'Robot: Memperbaiki kesalahan ejaan ke %(page)s',
    },
    # Author: Shirayuki
    'ja': {
        'misspelling-fixing': u'ロボットによる: 「%(page)s」へのリンクのスペル誤りを修正',
    },
    # Author: 아라
    'ko': {
        'misspelling-fixing': u'로봇: %(page)s 로의 링크 오타 고침',
    },
    # Author: Purodha
    'ksh': {
        'misspelling-fixing': u'Bot: Ene verkiehrt jetippte Lengk op di Sigg %(page)s repareert.',
    },
    # Author: Robby
    'lb': {
        'misspelling-fixing': u'Bot:Falschgeschriwwene Link %(page)s verbessert',
    },
    # Author: Bjankuloski06
    'mk': {
        'misspelling-fixing': u'Робот: Исправка на погрешно напишана врска: %(page)s',
    },
    # Author: Praveenp
    'ml': {
        'misspelling-fixing': u'യന്ത്രം: %(page)s എന്ന താളിലേയ്ക്കുള്ള അക്ഷരപിശകുള്ള കണ്ണി ശരിയാക്കുന്നു',
    },
    'nds': {
        'misspelling-fixing': u'Bot: rut mit verkehrt schreven Lenk op %(page)s',
    },
    # Author: Njsubedi
    'ne': {
        'misspelling-fixing': u'रोबोट: %(page)s को लागि हिज्जे बिग्रिएका लिङ्क सुधार गर्दै',
    },
    # Author: Siebrand
    'nl': {
        'misspelling-fixing': u'Robot: verkeerd gespelde verwijzing naar %(page)s gecorrigeerd',
    },
    # Author: Cocu
    'no': {
        'misspelling-fixing': u'Robot: Retter feilstavet lenke til %(page)s',
    },
    'pl': {
        'misspelling-fixing': u'Robot poprawia literówkę w linku do %(page)s',
    },
    'pt': {
        'misspelling-fixing': u'Bot: Corrigindo link com erro ortográfico para %(page)s',
    },
    # Author: Amgauna
    'pt-br': {
        'misspelling-fixing': u'Bot: Corrigindo link com erro ortográfico para %(page)s',
    },
    # Author: Okras
    'ru': {
        'misspelling-fixing': u'Робот: Исправление ссылки с ошибками на %(page)s',
    },
    # Author: John Reid
    'sco': {
        'misspelling-fixing': u'Robot: Fixing misspeelt link til %(page)s',
    },
    # Author: Andriykopanytsia
    'uk': {
        'misspelling-fixing': u'Робот: виправлення невірного посилання на %(page)s',
    },
    # Author: Minh Nguyen
    'vi': {
        'misspelling-fixing': u'Bot: Sửa chính tả trong liên kết đến %(page)s',
    },
};
